...
const NOVEL_STYLE_RULE = '문체는 반드시 소설체(예: "~했다", "~였다" 같은 평서형 과거시제 종결어미)로 통일하세요. "~했어요", "~합니다", "~했답니다" 같은 구어체·존댓말·감탄형 종결어미는 절대 쓰지 마세요. 이모지도 쓰지 마세요. 매 화마다 이 문체를 정확히 동일하게 유지해야 합니다.';
...
이건 소설이 아니라 신문 운세란입니다. 아래 규칙을 반드시 지키세요:
- 특정 인물이 등장하거나("낯선 이가", "귀인이 나타났다") 구체적 사건이 벌어지는("전보가 도착했다", "손님이 문을 두드렸다") 이야기식 서술은 절대 금지입니다. 그런 건 소설이지 운세가 아닙니다.
- 실제 신문 운세란처럼, 두루뭉술한 예측과 조언을 직접 화법으로 건네세요.
- 좋은 예시: title="뜻밖의 좋은 소식이 있겠습니다", body="오늘은 평소보다 대인관계에 신경 쓰면 좋은 하루입니다. 미뤄왔던 연락을 해보세요. 금전 문제는 신중하게 결정하시길 바랍니다."
- 나쁜 예시(절대 이렇게 쓰지 마세요): "안개 낀 골목에서 귀인을 만났다", "전보가 도착했다" 같은 구체적 사건 묘사.

아래 JSON 형식으로만 답하세요. 다른 설명은 붙이지 마세요.
{"title":"짧은 한 줄 헤드라인","body":"운세 본문 (2~3문장, 직접 화법 조언·예측)"}
${ERA_RULE}
${langLine}`;
  try {
    const raw = await generateWithProfile(prompt);
